...
class RelayClient:
    ...
    async def listen(self):
        async for message in self.ws:
            ...
            print(f"[{self.node_id}] Received:", data)
            if "rpc" in data:
...
# Test sample
async def main():
    client = RelayClient("nodeB", "ws://localhost:8765")
    # client = RelayClient("nodeB", "ws://42.96.12.119:8765")
    await client.connect()
    await asyncio.sleep(2)
    
    # call RPC và gửi kết quả cho socket để chuyển cho NodeA
    #??
    
    result = await client.call_rpc("nodeA", "ping", [])
    print("[nodeB] Got result:", result)
